  ...
  BasePage({ 
    ...
    getWbi() {
        this.request({
            ...
            .then((res) => {
              for(let i = 0; i < 8; i++) {
                textList[i].setProperty(hmUI.prop.TEXT, res.body.data[i].title)
                newTextList[i].setProperty(hmUI.prop.TEXT, res.body.data[i].owner.name)
                newNewTextList[i].setProperty(hmUI.prop.TEXT, formatNumber(res.body.data[i].stat.view))
