console.log('current filepath: /page/search.js');
try {
  (() => {
...
function createInputMethod(url) {
    ...
    const displayText = ui.createWidget(ui.widget.TEXT, {
        x: 60,
        y: 70,
        w: 350,
        h: 100,
    ...
    ui.createWidget(ui.widget.BUTTON, {
        ...
        h: 50,
        radius: 12,
        normal_color: 7039851,
        press_color: 16692392,
        text: ' ',
        click_func: button_widget => {
            if (currentInput) {
                if (chineseDictionary[currentInput]) {
                    inputText += chineseDictionary[currentInput][0];
                } else {
            ...
            displayText.setProperty(ui.prop.TEXT, inputText);
            updateCandidateDisplay();
        }
    });
    const candidateTextWidgets = [];
    for (let i = 0; i < 8; i++) {
        candidateTextWidgets.push(ui.createWidget(ui.widget.TEXT, {
            x: 20 + i * 45,
            y: 190,
            w: 50,
            h: 50,
        ...
        candidateTextWidgets[i].addEventListener(ui.event.CLICK_UP, () => {
            if (i < candidateWords.length) {
                inputText += candidateWords[i];
                currentInput = '';
                displayText.setProperty(ui.prop.TEXT, inputText);
                updateCandidateDisplay();
            }
    ...
    function updateCandidateDisplay() {
        for (let i = 0; i < 8; i++) {
            if (i < candidateWords.length) {
                candidateTextWidgets[i].setProperty(ui.prop.TEXT, candidateWords[i]);
            } else {
                candidateTextWidgets[i].setProperty(ui.prop.TEXT, '');
            }
        }
    }
    updateCandidateDisplay();
    const img_bkg = ui.createWidget(ui.widget.BUTTON, {
        x: 305,
        y: 420,
        text: '符',
        w: 50,
        h: 50,
        radius: 12,
        normal_color: 16542032,
        press_color: 16692392
    });
    img_bkg.addEventListener(ui.event.CLICK_DOWN, function (info) {
        console.log(info.x);
        switchUI(false);
    });
    ...
    ui.createWidget(ui.widget.BUTTON, {
        ...
        normal_color: 16542032,
        press_color: 16692392
    }).addEventListener(ui.event.CLICK_DOWN, function (info) {
        isEnglishMode = !isEnglishMode;
        displayText.setProperty(ui.prop.TEXT, inputText);
    });
    const buttonWidgets = buttons.map(button => {
        return ui.createWidget(ui.widget.BUTTON, {
            x: button.x,
            y: button.y,
            w: 40,
            h: 50,
            ...
            click_func: button_widget => {
                if (isEnglishMode) {
                    ...
                    currentInput += button.text.toLowerCase();
                    displayText.setProperty(ui.prop.TEXT, inputText + '[' + currentInput + ']');
                    if (chineseDictionary[currentInput]) {
                        candidateWords = chineseDictionary[currentInput];
                        updateCandidateDisplay();
                    }
                }
            }
    ...
    let txtBg = ui.createWidget(ui.widget.FILL_RECT, {
        x: 10,
        y: 228,
        w: 460,
        h: 185,
    ...
    txtBg.setProperty(ui.prop.VISIBLE, false);
    let txtGroup = ui.createWidget(ui.widget.GROUP, {
        x: 0,
        y: 0,
        w: 192,
        h: 490
    ...
    function switchUI(b) {
        img_bkg.setProperty(ui.prop.VISIBLE, b);
        txtBg.setProperty(ui.prop.VISIBLE, !b);
        txtGroup.setProperty(ui.prop.VISIBLE, !b);
        buttonWidgets.forEach(widget2 => widget2.setProperty(ui.prop.VISIBLE, b));
        if (b) {
            new ProxySc().show();
        } else {
            new ProxySc().hide();
    ...
    biaodianbuttons.map(button => {
        txtGroup.createWidget(ui.widget.BUTTON, {
            x: button.x,
            y: button.y,
            w: 40,
            h: 50,
    ...
    txtGroup.createWidget(ui.widget.BUTTON, {
        ...
        h: 50,
        radius: 20,
        press_color: 12829635,
        normal_color: 15724527,
        text: '返回',
        color: 0,
    ...
    txtGroup.setProperty(ui.prop.VISIBLE, false);
}
